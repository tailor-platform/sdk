...
describe("formatReportTable", () => {
  ...
  it("counts judge affordance labels into analytics.affordanceDistribution", () => {
    const failing: StageResult = { stage: "tests", passed: false, output: "fail" };
    const report = createReport([
      makeProblemResult({
        problemId: "001",
    ...
    expect(report.analytics.affordanceDistribution).toEqual({
      missing_action_verb: 2,
      context_bloat: 1,
    });
  });

  it("returns an empty affordanceDistribution when no result has a judge", () => {
    const passing: StageResult = { stage: "tests", passed: true, output: "ok" };
    const report = createReport([makeProblemResult({ stages: [passing], passed: true })]);
    expect(report.analytics.affordanceDistribution).toEqual({});
  });

  it("renders 'Top affordances' line in the formatted table when judge results exist", () => {
    const failing: StageResult = { stage: "tests", passed: false, output: "fail" };
    const report = createReport([
      makeProblemResult({
        problemId: "001",
    ...
    const table = formatReportTable(report);
    expect(table).toContain("Top affordances:");
    expect(table).toMatch(/missing_action_verb=1/);
    expect(table).toMatch(/context_bloat=1/);
  });

  it("records improvementCandidatesPath when supplied via metadata", () => {
    const failing: StageResult = { stage: "tests", passed: false, output: "fail" };
    const report = createReport([makeProblemResult({ stages: [failing], passed: false })], {
      improvementCandidatesPath: "artifacts/run/improvement-candidates.jsonl",
    });
    expect(report.analytics.improvementCandidatesPath).toBe(
      "artifacts/run/improvement-candidates.jsonl",
    );
  });
