import type { SolveAgent } from "./solver/types";

export function normalizeModelForAgent(agent: SolveAgent, model?: string): string | undefined {
  if (agent === "codex" && (!model || model === "default")) {
    return undefined;
  }
  return model;
}

export function formatSolveModelLabel(agent: SolveAgent, model?: string): string {
  const modelLabel = model ?? (agent === "claude" ? "sonnet" : "default");
  return `${agent}:${modelLabel}`;
}

export function parseSolveModelLabel(label?: string): { agent: SolveAgent; model?: string } {
  if (!label) {
    return { agent: "claude", model: "sonnet" };
  }
  const [prefix, ...rest] = label.split(":");
  if ((prefix === "claude" || prefix === "codex") && rest.length > 0) {
    return {
      agent: prefix,
      model: rest.join(":"),
    };
  }
  // Backward compatibility: previous reports stored only model without agent.
  return { agent: "claude", model: label };
}

export function resolveRerunSolveConfig(options: {
  reportModelRaw?: string;
  agent: SolveAgent;
  model?: string;
  agentExplicit: boolean;
  modelExplicit: boolean;
}): { agent: SolveAgent; model?: string } {
  const primaryReportModel = options.reportModelRaw?.split("+")[0];
  const parsedReportModel = parseSolveModelLabel(primaryReportModel);
  const rerunAgent = options.agentExplicit ? options.agent : parsedReportModel.agent;
  let rerunModel: string | undefined;
  if (options.modelExplicit) {
    rerunModel = options.model;
  } else if (options.agentExplicit && options.agent !== parsedReportModel.agent) {
    rerunModel = rerunAgent === "claude" ? "sonnet" : undefined;
  } else {
    rerunModel = parsedReportModel.model;
  }
  return { agent: rerunAgent, model: rerunModel };
}
