...
**Note**: This model only has `createdAt` (no `updatedAt`). Do NOT use `timestampFields()` - define `createdAt` manually as a datetime with a create hook setting it to the current timestamp.
