...
describe.skipIf(!workDirExists)("041-fix-broken-resolver", () => {
  ...
  test("body calculates gold membership correctly", async () => {
    const { default: resolver } = await import(resolverPath);
  ...
  test("output exists and is properly structured", async () => {
    const { default: resolver } = await import(resolverPath);
