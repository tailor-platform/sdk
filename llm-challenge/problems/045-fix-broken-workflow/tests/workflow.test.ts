...
describe.skipIf(!workDirExists)("045-fix-broken-workflow", () => {
  ...
  test("processOrder job name is 'process-order' (not 'validate-payment')", async () => {
    const mod = await import(workflowPath);
  ...
  test("processOrder body calls trigger and returns correct structure", async () => {
    ...
    expect(shipping).toEqual({ shipped: true, orderId: "ord-1", trackingId: "TRK-001" });
  });
});
