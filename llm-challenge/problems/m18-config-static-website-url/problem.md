...
1. Defines a static website named `"my-frontend"` (any non-empty description
   is fine) using the SDK's static-website factory — the one exported from
   `@tailor-platform/sdk` that takes the website's name as its first
   positional argument and a config object (with at least `description`) as
   its second. Hold the factory's return value in a local binding so the
   rest of the config can read the deferred deployment URL via `.url`.
2. Configures a single OAuth2 client called `"web"` whose redirect URI for
   `/callback` is anchored at that local binding's `.url` getter.
3. Sets the application's CORS allow-list to that same `.url` getter (the
   same expression — no duplicated string literal).
...
- The CORS entry and the OAuth2 redirect URI must both derive from the same
  static-website factory instance — no duplicated string literals.
...
client, and auth configuration shapes. No external documentation is required
for this task.
