...
- `--model <ollama-id>` — Ollama model id passed to opencode as `ollama/<id>`. Default: `gpt-oss:20b`.
...
A full 25-problem run at the defaults (`--iterations 5`, `--max-seconds 3600`, `--context-profile types-only`) takes roughly **6–19 hours** end-to-end on an Apple Silicon Mac with `gpt-oss:20b`; the wide band reflects how often `--max-seconds` is the binding constraint for the harder problems.
...
pnpm challenge:analyze --trend --model gpt-oss:20b --context-profile types-only
