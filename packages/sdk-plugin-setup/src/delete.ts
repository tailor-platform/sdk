...
function resolveMatch(outputDir: string, targets: readonly LockTarget[], input: string): Match {
  ...
  if (!target) {
    throw new Error(
      `"${relPath}" is not a managed workflow/action file (not recorded in .github/tailor.lock). ` +
        "Only files generated by `tailor setup` can be deleted with this command.",
    );
  }
...
/**
 * Delete managed workflow/action files and their `.github/tailor.lock` entries.
 *
