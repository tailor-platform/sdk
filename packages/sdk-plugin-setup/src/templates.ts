...
#
# - The SDK manages the jobs and steps whose id starts with \`tailor-\` and the
#   top-level keys it writes. Do not edit or rename them: \`tailor setup check\`
#   reports it, and re-running \`tailor setup\` then needs --force.
# - Re-running \`tailor setup\` keeps your own jobs, steps, and top-level keys,
#   plus runs-on, timeout-minutes, container, and env on managed jobs and the
#   ignore, fail-on-drift, install-command, node-version-file, label, and
#   user-mapping inputs of managed steps.
# - State is tracked in .github/tailor.lock (machine-owned: commit it; the only
#   part to edit by hand is appIds, to re-key an app after moving its directory).`;
...
export type RenderTagParams = {
  ...
  /** Include the migration-drift-check step in the plan job (default: false). */
  migrationDriftCheck?: boolean;
  /** Deploy on manual dispatch only from a tag (reachable from `branch` when set). */
  restrictDispatch?: boolean;
};
...
export type RenderCoordinateParams = {
  ...
  environment: string;
  packageManager: PackageManager;
  /** Restrict manual dispatch deploys as the branch/tag targets do. */
  restrictDispatch?: boolean;
};
...
function applyCommon(
  ...
  return out
    ...
    .replaceAll("__TAILOR_CLI__", () => TAILOR_CLI_COMMANDS[packageManager])
    .replaceAll("__APP_DIR__", () => workingDirectory ?? ".");
}

function branchDeployIf(restrictDispatch: boolean): string {
  const dispatch = restrictDispatch
    ? "(github.event_name == 'workflow_dispatch' && !inputs['dry-run'] && github.ref == 'refs/heads/__BRANCH__')"
    : "(github.event_name == 'workflow_dispatch' && !inputs['dry-run'])";
  return `if: |-\n  github.event_name == 'push' ||\n  ${dispatch}`;
}

function tagDeployGate(
  hasGuard: boolean,
  restrictDispatch: boolean,
): { needs: string; if: string } {
  if (!restrictDispatch) {
    return {
      needs: "needs: tailor-plan",
      if: `if: \${{ !(github.event_name == 'workflow_dispatch' && inputs['dry-run']) }}`,
    };
  }
  const conditions = [
    "!(github.event_name == 'workflow_dispatch' && inputs['dry-run'])",
    "startsWith(github.ref, 'refs/tags/')",
    ...(hasGuard ? ["needs.tailor-tag-guard.outputs.on-branch == 'true'"] : []),
  ];
  return {
    needs: hasGuard ? "needs: [tailor-tag-guard, tailor-plan]" : "needs: tailor-plan",
    if: `if: |-\n  ${conditions.join(" &&\n  ")}`,
  };
}
...
export function renderBranchWorkflow(params: RenderBranchParams): RenderResult {
  ...
  generatedIds.push(
    ...
    "tailor-deploy/tailor-slack-prereq",
...
export function renderTagWorkflow(params: RenderTagParams): RenderResult {
  ...
  const deployGate = tagDeployGate(hasGuard, params.restrictDispatch ?? false);
  out = line(out, "DEPLOY_NEEDS", deployGate.needs);
  out = line(out, "DEPLOY_IF", deployGate.if);
...
export function renderCoordinateWorkflow(params: RenderCoordinateParams): RenderResult {
  ...
  if (isTag) {
    ...
    const deployGate = tagDeployGate(branch !== undefined, params.restrictDispatch ?? false);
    out = line(out, "DEPLOY_NEEDS", deployGate.needs);
    out = line(out, "DEPLOY_IF", deployGate.if);
