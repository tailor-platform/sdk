...
import {
  type checkGitHub as CheckGitHub,
  findTargetDrift,
  resolveWithinRoot,
  type TargetState,
} from "./check";
...
import type { logger as Logger } from "@tailor-platform/sdk/cli";
...
describe("checkGitHub (integration)", () => {
  ...
  describe("in CI (isCI: true)", () => {
    ...
    aroundAll(async (runSuite) => {
      ...
      ({ checkGitHub } = await import("./check"));
      ({ logger } = await import("@tailor-platform/sdk/cli"));
      await runSuite();
      vi.doUnmock("std-env");
      vi.resetModules();
    });
    ...
    const check = (gitRunner: () => string = () => "origin/main") =>
      checkGitHub({ outputDir: testDir, gitRunner });
    ...
    test("does not emit a drift result marker for an operational error", async () => {
      ...
      expect(
        logSpy.mock.calls.some(([message]) =>
          message.startsWith("TAILOR_SETUP_CHECK_DRIFT_COUNT="),
        ),
      ).toBe(false);
    });
