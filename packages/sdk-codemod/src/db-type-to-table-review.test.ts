...
describe("db-type-to-table review findings", () => {
  ...
  test("reports local SDK db aliases for LLM review", async () => {
    ...
    await fs.promises.writeFile(
      ...
      [
        ...
        "let assignedSchema;",
        "assignedSchema = db;",
        "let nsAssignedSchema;",
        "nsAssignedSchema = sdk.db;",
        "",
        ...
        "});",
        "",
        'export const assigned = assignedSchema.type("Assigned", {',
        "  name: assignedSchema.string(),",
        "});",
        "",
        'export const nsAssigned = nsAssignedSchema.type("NsAssigned", {',
        "  name: nsAssignedSchema.string(),",
        "});",
        "",
      ].join("\n"),
      "utf-8",
      ...
      expect.objectContaining({
        ...
        findings: [
          ...
          },
          {
            ...
            line: 9,
            message: "Review SDK db alias usage and migrate db.type builder calls to db.table.",
            excerpt: "assignedSchema = db;",
          },
          {
            file: "tailordb.ts",
            line: 11,
            message: "Review SDK db alias usage and migrate db.type builder calls to db.table.",
            excerpt: "nsAssignedSchema = sdk.db;",
          },
        ],
      }),
    ]);
  ...
  test("reports default parameter SDK db aliases for LLM review", async () => {
    tmpDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "db-type-review-test-"));
    await fs.promises.writeFile(
      path.join(tmpDir, "tailordb.ts"),
      [
        ...
        'import * as sdk from "@tailor-platform/sdk";',
        "",
        "export function makeUser(schema = db) {",
        '  return schema.type("User", {',
        "    name: schema.string(),",
        "  });",
        "}",
        "",
        "export const makeTeam = (schema = sdk.db) =>",
        '  schema.type("Team", {',
        "    name: schema.string(),",
        "  });",
        "",
        ...
        findings: [
          {
            ...
            line: 4,
            message: "Review SDK db alias usage and migrate db.type builder calls to db.table.",
            excerpt: "export function makeUser(schema = db) {",
          },
          {
            file: "tailordb.ts",
            line: 10,
            message: "Review SDK db alias usage and migrate db.type builder calls to db.table.",
            excerpt: "export const makeTeam = (schema = sdk.db) =>",
          },
        ],
      }),
    ]);
