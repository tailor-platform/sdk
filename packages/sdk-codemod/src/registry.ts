...
export const allCodemods: CodemodPackage[] = [
  ...
  },
  {
    ...
    examples: [
      ...
      {
        before: 'import { generator } from "./tailor.config";',
        after: 'import { plugins } from "./tailor.config";',
        caption: "Also updates other files that import the export by its old name.",
      },
    ],
    prompt: [
  ...
  },
  {
    ...
    // same-named variable.
    filePatterns: ["**/*.{ts,tsx,mts,cts,js,jsx,mjs,cjs}"],
    suspiciousPatterns: [/\bsecrets\.get(?:All)?\(/, /\bsecrets\s+as\s+\w+/, /=\s*secrets\b/],
    examples: [
      {
        before:
