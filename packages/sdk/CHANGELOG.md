...
- [#2370](https://github.com/tailor-platform/sdk/pull/2370) [`5ecfb0f`](https://github.com/tailor-platform/sdk/commit/5ecfb0fe27e7d72dcea4421c5c8426ca9689284f) Thanks [@toiroakr](https://github.com/toiroakr)! - `defineConfig({ env })` values now accept the `<name>:url` static website placeholder, the same one `cors`, OAuth2 redirect URIs, and IdP return origins already accept. On deploy, the CLI replaces such a value with the deployed website's URL before it reaches resolver, executor, workflow job, auth hook, and TailorDB migration script code, so `env.siteUrl` reads the site's actual deployed URL (e.g. `https://my-site.example.com`) at runtime instead of the literal string `"my-site:url"`. A path suffix works too (`"my-site:url/callback"`).
  ...
  TailorDB's pre-deploy migration state check now always runs first, before any resource is created or updated, so a stale migration checkpoint or schema fails the deploy without leaving other resources partially applied.

### Patch Changes

- [#2382](https://github.com/tailor-platform/sdk/pull/2382) [`6b0fad4`](https://github.com/tailor-platform/sdk/commit/6b0fad4fa0cbd1140abde7538f334537e2a229e6) Thanks [@renovate](https://github.com/apps/renovate)! - fix(deps): update dependency @​clack/prompts to v1.8.1

- [#2391](https://github.com/tailor-platform/sdk/pull/2391) [`f92a680`](https://github.com/tailor-platform/sdk/commit/f92a680a86d1eebfdc9d7b1a409f084373ef3067) Thanks [@renovate](https://github.com/apps/renovate)! - fix(deps): update dependency open to v11.0.4
...
- [#2375](https://github.com/tailor-platform/sdk/pull/2375) [`e54b854`](https://github.com/tailor-platform/sdk/commit/e54b85445c64ba30cb19731d1122488d6826e1b5) Thanks [@toiroakr](https://github.com/toiroakr)! - Fix a bug where the CLI could unnecessarily require a full `tailor login` sooner than expected. When a token refresh succeeded but the server did not issue a new refresh token, the CLI was overwriting the still-valid saved refresh token with nothing, so the very next time the access token expired there was no refresh token left to try and the CLI forced a fresh login instead of attempting one more refresh.

- [#2318](https://github.com/tailor-platform/sdk/pull/2318) [`b8f397a`](https://github.com/tailor-platform/sdk/commit/b8f397a08941f78bf670cb806287b20427fe1f10) Thanks [@toiroakr](https://github.com/toiroakr)! - Preserve field hooks and boolean validator behavior when replaying migration snapshots and diffs generated by older SDKs, including existing field values during partial updates, nested fields, and the first validation error. Nested hooks retain omitted parents and respect explicit nulls and replacement objects or arrays, and nested validators read sibling fields from their own containing object or array element rather than the top-level record. Avoid false script-drift errors when deploying existing histories and preserve validation metadata when rebaselining.

- [#2354](https://github.com/tailor-platform/sdk/pull/2354) [`4096ad5`](https://github.com/tailor-platform/sdk/commit/4096ad5aad03a2dd9aef6fd334334bfcb60aaff4) Thanks [@renovate](https://github.com/apps/renovate)! - fix(deps): update dependency open to v11.0.3

- [#2355](https://github.com/tailor-platform/sdk/pull/2355) [`aa99ee5`](https://github.com/tailor-platform/sdk/commit/aa99ee5af9c94fe7bfaa8c8c13c42f67db517763) Thanks [@renovate](https://github.com/apps/renovate)! - fix(deps): update dependency rolldown to v1.2.8

- [#2361](https://github.com/tailor-platform/sdk/pull/2361) [`7361bbf`](https://github.com/tailor-platform/sdk/commit/7361bbf1aa0dd608548d637213a9bcf7286aeb94) Thanks [@renovate](https://github.com/apps/renovate)! - fix(deps): update dependency @bufbuild/protobuf to v2.15.0
...
- [#2367](https://github.com/tailor-platform/sdk/pull/2367) [`a7b581f`](https://github.com/tailor-platform/sdk/commit/a7b581f1909d85eb61e06b783a6a2abd8658b0a2) Thanks [@renovate](https://github.com/apps/renovate)! - fix(deps): update oxc

## 2.17.0

### Minor Changes

- [#2330](https://github.com/tailor-platform/sdk/pull/2330) [`3e8dc24`](https://github.com/tailor-platform/sdk/commit/3e8dc24bf2a5b55c811e28570dffb763baf9030e) Thanks [@toiroakr](https://github.com/toiroakr)! - Add `logger.registerSecret(value)` to the CLI logger (`@tailor-platform/sdk/cli`). Values registered this way are automatically replaced with `<redacted>` in diagnostic log output (`info`/`success`/`warn`/`error`/`log`/`debug`), as a technical safety net on top of the existing convention of not passing secret values to the logger. `logger.out()` is unaffected, so commands that intentionally print a secret as their primary result (machine user tokens, OAuth2 client secrets) are unchanged. The CLI itself now registers access tokens, refresh tokens, OAuth2/IdP client secrets, and Secret Manager values as soon as they enter the process. Crash reports (written locally, and optionally sent remotely, when an unexpected error occurs) also redact registered secrets, in addition to their existing pattern-based sanitization of UUIDs, hex tokens, emails, and paths.
...
- [#2337](https://github.com/tailor-platform/sdk/pull/2337) [`f3289dd`](https://github.com/tailor-platform/sdk/commit/f3289dd9e373e479de8a1e0a9da8b026422d6fca) Thanks [@dqn](https://github.com/dqn)! - Write `db.pglite.ts` next to every generated migration `db.ts` with the `CREATE TABLE` script of the schema `migrate.ts` runs against, and let `tailordb migration script --with-test` also scaffold `migrate.pglite.test.ts` when `@electric-sql/pglite` is installed
  
  Generate migration types that accept nested objects and object arrays instead of JSON strings.

- [#2335](https://github.com/tailor-platform/sdk/pull/2335) [`39f32f6`](https://github.com/tailor-platform/sdk/commit/39f32f64f3292e57691d6469fd81cecda5f45ee7) Thanks [@dqn](https://github.com/dqn)! - Add `pgliteSchemaPath` to `kyselyTypePlugin` to generate the PGlite `CREATE TABLE` script for every TailorDB table alongside the Kysely types, so tests using `mockTailordbWithPGlite` or `createKyselyPGlite` no longer hand-write DDL
...
- [#2368](https://github.com/tailor-platform/sdk/pull/2368) [`4a03de8`](https://github.com/tailor-platform/sdk/commit/4a03de85d81f29fed5023b6ae6dab206521d0e90) Thanks [@dqn](https://github.com/dqn)! - Forward global flags typed before a CLI plugin command to the plugin.
...
- [#2338](https://github.com/tailor-platform/sdk/pull/2338) [`98a9615`](https://github.com/tailor-platform/sdk/commit/98a96153bda73cbbb72f8b6090c948eef6ed0c13) Thanks [@renovate](https://github.com/apps/renovate)! - chore(deps): lock file maintenance
...
- [#2342](https://github.com/tailor-platform/sdk/pull/2342) [`54eafd9`](https://github.com/tailor-platform/sdk/commit/54eafd96ff0d860ca39674acc36e26839e338850) Thanks [@dqn](https://github.com/dqn)! - Stop seed validation from reporting missing required fields inside a nested object that a row leaves out or gives a non-object value, and report the wrong shape against the field itself instead. Applies to both single nested objects and elements of a nested array.
