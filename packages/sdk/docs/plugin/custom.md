...
The SDK provides a `getGeneratedType()` helper function to retrieve plugin-generated TailorDB types. This enables generators and other tools to work with types generated by plugins.

```typescript
import { getGeneratedType } from "@tailor-platform/sdk/plugin";
import plugin from "@tailor-platform/sdk/changeset-plugin";
import { purchaseOrder } from "./tailordb/purchaseOrder";

// Get the generated type by plugin, source type, and kind
const ChangeRequest = getGeneratedType(plugin, purchaseOrder, "request");
```

**Parameters:**

- `plugin`: The plugin instance (must be default-exported)
- `sourceType`: The TailorDB type that the plugin is attached to
...
const PurchaseOrderChangeRequest = getGeneratedType(plugin, purchaseOrder, "request");

// Use the type in generators, tests, or other tools...
```
