...
| [`tailordb migration validate`](#tailordb-migration-validate) | Validate migration files and detect schema drift (local types vs. migration snapshot, remote schema vs. migration checkpoint) without deploying. Runs the same checks as 'deploy' and exits with a non-zero code when issues are found. |
