...
| `--older-than <OLDER_THAN>`           | -     | Minimum age since creation, such as 30m, 24h, or 7d. 0s disables the age check and requires a scope. Required unless --expired is given         | No       | -       | -                                 |
| `--expired`                           | -     | Select workspaces whose own --ttl expiry has passed, instead of by name and age. Requires --organization-id, --folder-id, or --personal         | No       | `false` | -                                 |
| `--personal`                          | -     | Also consider workspaces belonging to no organization and no folder, which --organization-id and --folder-id can never match. Counts as a scope | No       | `false` | -                                 |
