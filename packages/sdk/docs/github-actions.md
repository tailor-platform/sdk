...
runs the check in CI. A single run audits every target recorded in
`.github/tailor.lock`, so per-app composite actions generated by `setup action`
are covered by their coordinator's step and do not carry one of their own.
