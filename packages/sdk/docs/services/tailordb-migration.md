...
Migration scripts run as a workflow job on the platform, and `tailor deploy` waits for that job to finish. If the deploy is interrupted while the job is still running (for example with Ctrl+C), the job keeps running on the platform; the next `tailor deploy` notices it and waits for its outcome instead of starting the script a second time. It refuses to proceed only when it cannot safely adopt the job — the running job executes a different version of `migrate.ts` than the working tree, or several jobs of the same migration are still unfinished — and tells you to wait for the job to finish before retrying.

Two deploys of the same application never run concurrently (see [Concurrent Deploys](../cli/application.md#deploy)), so a migration cannot be started by one deploy while another is still applying it.
