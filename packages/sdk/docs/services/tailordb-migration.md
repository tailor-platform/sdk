...
- Deploys of one application to one workspace are serialized automatically (see [Concurrent Deploys](../cli/application.md#deploy)); a second deploy waits for the first. Applications that share tables still need their deploys coordinated by you.
...
Migration scripts run as a workflow job on the platform, and `tailor deploy` waits for that job to finish. If the deploy is interrupted while the job is still running (for example with Ctrl+C), the job keeps running on the platform. A `tailor deploy` started while that job is still running waits for its outcome instead of starting the script a second time; it refuses to proceed when it cannot safely adopt the job — the job cannot be confirmed to run the same script as the working tree, several jobs of the same migration are still unfinished, or the job is suspended waiting to be resumed — and tells you to wait for the job to finish before retrying. This applies when the interrupted deploy had not changed the schema yet (a data-only migration, or a run interrupted before its schema step); once the schema step has been applied, the next deploy reports the half-applied change as remote schema drift first, and you resolve it as described below. A job that already finished by the time the next deploy runs is treated like any other interrupted migration: the script runs again, which is why scripts should tolerate re-execution (see above).

Deploys of the same application are serialized (see [Concurrent Deploys](../cli/application.md#deploy)), so a migration is not started by one deploy while another is still applying it.
