...
When `migrate.ts` already exists (the usual case for breaking changes, where `migration generate` creates it), the command adds only `migrate.test.ts` — and if the migration's `db.ts` was generated by an older SDK version that did not export `Database`, updates it in place. Or write the test by hand:
