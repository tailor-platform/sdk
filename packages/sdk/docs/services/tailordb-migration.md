...
The checkpoint is advanced only after the reversible post-migration schema updates succeed. If the checkpoint write reports an error, the SDK reads it back: a matching value is treated as committed, while a confirmed old value triggers schema restoration. A value beyond the current migration indicates a concurrent deploy; the SDK aborts without rolling back the newer schema. If read-back also fails, the SDK leaves the post-migration schema unchanged rather than risk rolling it back past a committed checkpoint; verify the remote checkpoint before retrying.
