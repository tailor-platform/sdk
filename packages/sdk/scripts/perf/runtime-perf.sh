...
if ! TAILOR_PLATFORM_SDK_BUILD_ONLY=true pnpm exec tailor-sdk deploy -c tailor.config.ts > "${LOG_DIR}/apply-warmup.log" 2>&1; then
...
for i in $(seq 1 $ITERATIONS); do
  ...
  START=$(get_timestamp_ms)
  if ! TAILOR_PLATFORM_SDK_BUILD_ONLY=true pnpm exec tailor-sdk deploy -c tailor.config.ts > "${LOG_DIR}/apply-iter-${i}.log" 2>&1; then
