...
describe("loadConfig", () => {
  test("rejects a credential in env", async () => {
  ...
  test("resolves an allowed entry to its value, so the reason never travels with it", async () => {
    const configPath = writeConfig(
      `export default {
        name: "test-app",
        env: {
          SLACK_BOT_TOKEN: { value: ${JSON.stringify(SLACK_TOKEN)}, allowSecret: "demo workspace" },
          RETRIES: 3,
        },
      };`,
    );
  ...
  test("requires an allowed entry to state a reason", async () => {
    const configPath = writeConfig(
      `export default {
        name: "test-app",
        env: { SLACK_BOT_TOKEN: { value: ${JSON.stringify(SLACK_TOKEN)}, allowSecret: "" } },
      };`,
    );

    await expect(loadConfig(configPath)).rejects.toThrow(
      /'allowSecret' must state why the value is safe to keep in 'env'/,
    );
  });
