...
interface ExtractedAttributes {
  ...
  secretVaultNames?: Record<string, string[]>;
}
...
export function generateTypeDefinition(
  ...
  secretVaultNames?: Readonly<Record<string, readonly string[]>>,
): string {
...
${authNamespaceNameFields}
  ...
  // Generate SecretVaultNameRegistry interface. Each field's value is a union of
  // that vault's secret *names*, never the secret values — a value would leak the
  // configured secret into this generated file (same reasoning as Env above).
  const secretVaultEntries = secretVaultNames ? Object.entries(secretVaultNames) : [];
  const secretVaultFields = secretVaultEntries.length
    ? secretVaultEntries
        .map(([vault, names]) => {
          const key = isValidIdentifier(vault) ? vault : JSON.stringify(vault);
  ...
  const secretVaultBody =
    secretVaultEntries.length === 0
      ? "{}"
      : `{
${secretVaultFields}
  }`;
...
declare module "@tailor-platform/sdk" {
  ...
  interface SecretVaultNameRegistry ${secretVaultBody}
}
...
function collectAttributesFromConfig(config: AppConfig): ExtractedAttributes {
  ...
  // Extract Secret Manager vault -> secret *name* mapping. Only `Object.keys()` of
  // each vault, never its values — a value is the configured secret itself.
  const secretVaultNames = config.secrets
    ? Object.fromEntries(
        Object.entries(config.secrets.vaults).map(([vault, secrets]) => [
          vault,
          Object.keys(secrets),
        ]),
      )
  ...
  if (!auth || typeof auth !== "object") {
    return { idpNames, aiGatewayNames, authNamespaceNames, secretVaultNames };
  }
...
export async function generateUserTypes(options: GenerateUserTypesOptions): Promise<void> {
  ...
  try {
    ...
    if (secretVaultNames && Object.keys(secretVaultNames).length) {
      // Names only, never secret values.
      logger.debug(`Extracted SecretVaultNames: ${JSON.stringify(secretVaultNames)}`);
    }
