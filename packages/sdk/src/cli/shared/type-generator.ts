...
export function extractAttributesFromConfig(config: AppConfig): ExtractedAttributes {
  return { ...collectAttributesFromConfig(config), hasConfigId: Boolean(config.id) };
}
 ...
 * @param hasConfigId - Whether `tailor.config.ts` has a resolved `id` (used to require `id` on `defineConfig()`)
...
${aiGatewayNameFields}
  ...
  // Generate ConfigIdRegistry interface. Only the fact that an id has been
  // resolved is recorded, never the id's value.
  const configIdBody = hasConfigId
    ? `{
    hasId: true;
  }`
    : "{}";
...
declare module "@tailor-platform/sdk" {
  ...
  interface ConfigIdRegistry ${configIdBody}
}
