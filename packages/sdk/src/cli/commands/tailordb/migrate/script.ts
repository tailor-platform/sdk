...
export interface AddMigrationScriptFilesResult {
  ...
  testPath?: string;
  /** Existing db.ts path when it was updated to export Database for the test scaffold. */
  updatedDbTypesPath?: string;
}
...
export async function addMigrationScriptFiles(
  ...
  if (migrateExists && withTest) {
    // The test scaffold imports Database from ./db; db.ts files generated by
    // older SDK versions declare the interface without exporting it.
    ...
    if (
      !/^export interface Database\b/m.test(dbTypesContent) &&
      /^interface Database\b/m.test(dbTypesContent)
    ) {
      await fsPromises.writeFile(
        dbTypesPath,
        dbTypesContent.replace(/^interface Database\b/m, "export interface Database"),
      );
      result.updatedDbTypesPath = dbTypesPath;
    }
  }
...
async function script(options: ScriptOptions): Promise<void> {
  ...
  if (result.updatedDbTypesPath) {
    logger.info(`  Updated to export Database: ${result.updatedDbTypesPath}`);
  }
