...
function applyCommon(
  ...
  const { folderId, workingDirectory, environment, packageManager } = params;
  let out = line(content, "HEADER", HEADER);
  out = line(out, "FOLDER_ID", folderId ? `folder-id: ${folderId}` : undefined);
  out = line(
    out,
    "WORKING_DIRECTORY",
    workingDirectory ? `working-directory: ${workingDirectory}` : undefined,
  );
  out = line(out, "ENVIRONMENT", environment ? `environment: ${environment}` : undefined);
  ...
  return out
    ...
    .replaceAll("__WORKSPACE_REGION__", () => params.workspaceRegion)
    .replaceAll("__ORGANIZATION_ID__", () => params.organizationId)
