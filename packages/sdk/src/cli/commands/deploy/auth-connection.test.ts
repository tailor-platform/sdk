  ...
  return {
    ...
    getMetadata: vi.fn().mockImplementation(({ trn }: { trn: string }) => {
      if (!opts.metadataSupported) {
        throw new ConnectError("unsupported resource type", Code.InvalidArgument);
      }
...
describe("planAuthConnections with platform metadata support", () => {
  beforeEach(() => {
...
describe("planAuthConnections fallback when metadata is unsupported", () => {
  beforeEach(() => {
    mockLoadSecretsState.mockReset();
  });
