...
import {
  assertEventSubscriptionsAreDeclared,
...
type FakeEventSourceTarget = {
  configPath: string;
  /** Namespace this config declares its TailorDB types and resolvers in. */
  namespace?: string;
  /** TailorDB types declared by this config, with their `publishEvents` value. */
  types?: Record<string, boolean | undefined>;
  /** Resolvers declared by this config, with their `publishEvents` value. */
  resolvers?: Record<string, boolean | undefined>;
  /** IdPs declared by this config, with their `publishEvents` value. */
  idps?: Record<string, boolean | undefined>;
  /** Workflows declared by this config, with their `publishEvents` value. */
  workflows?: Record<string, boolean | undefined>;
  /** Executors declared by this config, keyed by executor name. */
  executors?: Record<string, Record<string, unknown>>;
};

function fakeEventSourceTarget(
  spec: FakeEventSourceTarget,
): Parameters<typeof assertEventSubscriptionsAreDeclared>[0][number] {
      ...
      tailorDBServices: [
        {
          ...
          types: Object.fromEntries(
            Object.entries(spec.types ?? {}).map(([name, publishEvents]) => [
              name,
              { name, settings: { publishEvents } },
            ]),
          ),
        },
      ],
  ...
  } as unknown as Parameters<typeof assertEventSubscriptionsAreDeclared>[0][number];
}

describe("assertEventSubscriptionsAreDeclared", () => {
  test("accepts an executor subscribing to a type its own config declares", () => {
    expect(() =>
      assertEventSubscriptionsAreDeclared([
        fakeEventSourceTarget({
          configPath: "buyer/tailor.config.ts",
          types: { Order: undefined },
  ...
  test.each([
    {
      kind: "TailorDB type",
      owner: { configPath: "supplier/tailor.config.ts", types: { Order: undefined } },
      trigger: { kind: "tailordb", typeName: "Order" },
      resource: 'TailorDB type "Order"',
    },
    {
      kind: "Resolver",
      owner: { configPath: "supplier/tailor.config.ts", resolvers: { processOrder: undefined } },
      trigger: { kind: "resolverExecuted", resolverName: "processOrder" },
      resource: 'Resolver "processOrder"',
    },
    {
      kind: "IdP service",
      owner: { configPath: "supplier/tailor.config.ts", idps: { "shared-idp": undefined } },
      trigger: { kind: "idpUser", idp: "shared-idp" },
      resource: 'IdP service "shared-idp"',
    },
  ])("throws for a cross-config $kind subscription left unset", ({ owner, trigger, resource }) => {
    expect(() =>
      assertEventSubscriptionsAreDeclared([
        fakeEventSourceTarget(owner),
        fakeEventSourceTarget({
          configPath: "buyer/tailor.config.ts",
          executors: { "sync-order": trigger },
        }),
      ]),
    ).toThrow(
      `Executor "sync-order" subscribes to ${resource}, declared by supplier/tailor.config.ts. ` +
        `Set "publishEvents: true" on ${resource}`,
    );
  });

  test("accepts a cross-config subscription when the owner opts in explicitly", () => {
    expect(() =>
      assertEventSubscriptionsAreDeclared([
        fakeEventSourceTarget({
  ...
  test("reports a cross-config opt-out the way a same-config opt-out is reported", () => {
    expect(() =>
      assertEventSubscriptionsAreDeclared([
        fakeEventSourceTarget({
          configPath: "supplier/tailor.config.ts",
      ...
      'TailorDB type "Order" has "publishEvents: false", but executors with a record trigger subscribe to it.',
    );
  });

  describe("workflow execution events", () => {
    test.each([{ kind: "workflowExecution" }, { kind: "workflowJobExecution" }])(
      "throws when no config in the run declares the subscribed workflow ($kind)",
      ({ kind }) => {
        expect(() =>
          assertEventSubscriptionsAreDeclared([
            fakeEventSourceTarget({
          ...
          'Executor "sync-orders" subscribes to Workflow "orders", which no config in this deploy declares.',
        );
      },
    );

    test("accepts a workflow its own config declares", () => {
      expect(() =>
        assertEventSubscriptionsAreDeclared([
          fakeEventSourceTarget({
            configPath: "buyer/tailor.config.ts",
            workflows: { orders: undefined },
            executors: {
              "sync-orders": { kind: "workflowExecution", workflowName: "orders" },
            },
          }),
        ]),
      ).not.toThrow();
    ...
    test("requires a cross-config workflow to opt in explicitly", () => {
      expect(() =>
        assertEventSubscriptionsAreDeclared([
          fakeEventSourceTarget({
            configPath: "supplier/tailor.config.ts",
      ...
      ).toThrow('Set "publishEvents: true" on Workflow "orders"');
    });

    test("only verifies presence for a job execution subscription", () => {
      // The flag lives on each job the workflow runs, which this check cannot read.
      expect(() =>
        assertEventSubscriptionsAreDeclared([
          fakeEventSourceTarget({
            configPath: "supplier/tailor.config.ts",
  ...
  test("throws when no config in the run declares the subscribed resource", () => {
    expect(() =>
      assertEventSubscriptionsAreDeclared([
        fakeEventSourceTarget({
          configPath: "buyer/tailor.config.ts",
      ...
      'Executor "sync-order" subscribes to TailorDB type "Order", which no config in this deploy declares. ' +
        'Add the config that owns it to --config so its "publishEvents" can be verified.',
    );
  });

  test("infers the subscribed IdP when the config declares exactly one", () => {
    expect(() =>
      assertEventSubscriptionsAreDeclared([
        fakeEventSourceTarget({
          configPath: "supplier/tailor.config.ts",
        ...
        fakeEventSourceTarget({
          configPath: "buyer/tailor.config.ts",
          idps: { "buyer-idp": undefined },
          executors: { "sync-user": { kind: "idpUser" } },
        }),
      ]),
    ).not.toThrow();
  });
  ...
  test.each([
    { order: "owner first", reversed: false },
    { order: "owner last", reversed: true },
  ])(
    "accepts subscribing to its own resolver when a peer namespace reuses the name ($order)",
    ({ reversed }) => {
      const peer = fakeEventSourceTarget({
        configPath: "buyer/tailor.config.ts",
        namespace: "buyer",
        resolvers: { get: undefined },
      });
      const subscriber = fakeEventSourceTarget({
        configPath: "supplier/tailor.config.ts",
        namespace: "supplier",
        resolvers: { get: undefined },
        executors: { "sync-get": { kind: "resolverExecuted", resolverName: "get" } },
      });

      expect(() =>
        assertEventSubscriptionsAreDeclared(reversed ? [subscriber, peer] : [peer, subscriber]),
      ).not.toThrow();
    },
  );

  test("accepts subscribing to its own TailorDB type when a peer namespace reuses the name", () => {
    expect(() =>
      assertEventSubscriptionsAreDeclared([
        fakeEventSourceTarget({
          configPath: "supplier/tailor.config.ts",
  ...
  test("stays quiet when two peer namespaces both declare the subscribed name", () => {
    // Which namespace the trigger means is reported when the executor's
    // namespace is resolved, so this check must not guess an owner.
    expect(() =>
      assertEventSubscriptionsAreDeclared([
        fakeEventSourceTarget({
          configPath: "buyer/tailor.config.ts",
  ...
  test("ignores triggers that no resource publishes for", () => {
    expect(() =>
      assertEventSubscriptionsAreDeclared([
        fakeEventSourceTarget({
          configPath: "buyer/tailor.config.ts",
          executors: { "nightly-sync": { kind: "schedule" } },
        }),
      ]),
    ).not.toThrow();
  });
