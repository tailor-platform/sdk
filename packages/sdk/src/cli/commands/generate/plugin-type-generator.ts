    ...
    if (existing) {
      throw new Error(
        `Duplicate plugin-generated table name "${info.type.name}" detected. ` +
          `First: plugin "${existing.pluginId}" (kind: "${existing.kind}", source table: "${existing.sourceTypeName}"), ` +
          `Second: plugin "${info.pluginId}" (kind: "${info.kind}", source table: "${info.sourceTypeName}"). ` +
