...
describe("user list", () => {
  ...
  test("marks only the default token current when the same user also has a scoped token", async () => {
    writePlatformConfig({
      version: 2,
      min_sdk_version: "1.29.0",
      users: {
    ...
    expect(stderr.output).toContain("u@example.com [https://api.dev.tailor.tech]");
    expect(stderr.output).not.toContain("u@example.com [https://api.dev.tailor.tech] (current)");
  });
  ...
  test("does not mark a default token current for an explicit profile platform URL", async () => {
    vi.stubEnv("TAILOR_PLATFORM_PROFILE", "dev");
    writePlatformConfig({
      version: 2,
      min_sdk_version: "1.29.0",
