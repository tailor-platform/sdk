...
import { extractColumnTemplate, extractTableNamesFromSql } from "./sql-type-extractor";
...
describe("extractTableNamesFromSql", () => {
  test("extracts table names using SQL parser", () => {
    ...
    expect(extractTableNamesFromSql(sql)).toEqual(["User", "Order"]);
  });
  ...
  test("does not throw for parseable non-DML statements", () => {
    ...
    expect(() => extractTableNamesFromSql(sql)).not.toThrow();
  });
});
...
describe("extractColumnTemplate", () => {
  ...
  test("returns wildcard slot with all types for SELECT * with JOIN", () => {
    ...
    ).toEqual([{ type: "wildcard", tableNames: ["User", "Order"] }]);
  });
  ...
  test("returns only wildcard table for qualified wildcard in JOIN", () => {
    ...
    ).toEqual([{ type: "wildcard", tableNames: ["User"] }]);
  });
  ...
  test("returns separate wildcard slots in declaration order", () => {
    expect(
      ...
      { type: "wildcard", tableNames: ["Order"] },
      { type: "wildcard", tableNames: ["User"] },
    ]);
  });
