...
describe("FileUtilsGenerator", () => {
  ...
  describe("generateFileUtils", () => {
    it("should generate TypeWithFiles interface", async () => {
      const types = {
        User: {
          name: "User",
          fileFields: ["avatar"],
      ...
      const result = await FileProcessor.generateFileUtils(types, "tailordb");
    ...
    it("should generate union type for multiple fields", async () => {
      const types = {
        SalesOrder: {
          name: "SalesOrder",
          fileFields: ["receipt", "form"],
        },
      };
    ...
    it("should return empty string when no files are present", async () => {
      const types = {
        User: {
          name: "User",
          fileFields: [],
    ...
    it("should handle multiple types with files", async () => {
      const types = {
        User: {
          name: "User",
          fileFields: ["avatar"],
    ...
    it("should generate namespaces object and downloadFile helper", async () => {
      const types = {
        User: {
          name: "User",
          fileFields: ["avatar"],
      ...
      expect(result).toContain("export async function downloadFile");
      expect(result).toContain(
        "tailordb.file.download(namespaces[type], type, field, recordId)",
      );
    });
  });
