...
import { ExecutorLoader } from "./loader";
...
/**
 * Simplified executor bundler that:
 * 1. Creates entry file that extracts operation.body
 * 2. Bundles in a single step with tree-shaking
 */
export class ExecutorBundler {
  private loader = new ExecutorLoader();

  constructor(private config: FileLoadConfig) {}
  ...
  async bundle(): Promise<void> {
    ...
    for (const file of files) {
      const executor = await this.loader.load(file);
      if (!executor) {
    ...
    await Promise.all(
      executors.map((executor) =>
        this.bundleExecutor(executor, entryDir, outputDir, tsconfig),
      ),
    );
