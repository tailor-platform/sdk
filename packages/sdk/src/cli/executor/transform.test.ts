...
describe("transform", () => {
  ...
  describe("toExecutorInfo", () => {
    ...
    test("returns raw triggerType in jsonMode", () => {
      const executor = {
        name: "scheduled-executor",
        description: "A scheduled executor",
        triggerType: ExecutorTriggerType.SCHEDULE,
      ...
      const result = toExecutorInfo(executor, { jsonMode: true });

      expect(result.triggerType).toBe("SCHEDULE");
    });
  });

  describe("toExecutorListInfo with jsonMode", () => {
    test("returns raw triggerType in jsonMode", () => {
      const executor = {
        name: "event-executor",
        triggerType: ExecutorTriggerType.EVENT,
      ...
      const result = toExecutorListInfo(executor, { jsonMode: true });

      expect(result.triggerType).toBe("EVENT");
    });

    test("returns formatted triggerType without jsonMode", () => {
      const executor = {
        name: "event-executor",
        triggerType: ExecutorTriggerType.EVENT,
        triggerConfig: {
