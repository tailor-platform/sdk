 ...
 * @param pluginIds - Array of plugin IDs being used
 * @returns Generated type definition source
 */
export function generatePluginTypeDefinition(pluginIds: string[]): string {
...
`;
  ...
  // Generate placeholder config types for each plugin
  const pluginEntries = pluginIds.map((id) => `    "${id}": Record<string, unknown>;`).join("\n");
...
// Regenerated automatically when running 'tailor-sdk generate'
//
// To add type-safe plugin configurations, replace Record<string, unknown>
// with your plugin's specific config type:
//
// declare module "@tailor-platform/sdk" {
//   interface PluginConfigs {
//     "@my-plugin/name": {
 ...
 * @returns Promise that resolves when types are generated
 */
export async function generatePluginTypes(pluginIds: string[], configPath: string): Promise<void> {
  try {
    if (pluginIds.length === 0) {
      logger.debug("No plugins registered, generating empty plugin-defined.d.ts");
    } else {
      logger.debug(`Generating plugin types for: ${pluginIds.join(", ")}`);
    }

    const typeDefContent = generatePluginTypeDefinition(pluginIds);
