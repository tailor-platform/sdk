...
describe("AppConfigSchema", () => {
  test("accepts a minimal valid config", () => {
    const result = v.safeParse(AppConfigSchema, { name: "my-app" });
  ...
  test("rejects an id that is not a UUID", () => {
    ...
    expect(result.issues[0].path?.[0]?.key).toEqual("id");
  });
  ...
  test("rejects when name is an empty string", () => {
    const result = v.safeParse(AppConfigSchema, { name: "" });
    expect(result.success).toBe(false);
  });
  ...
  test("rejects unsupported log levels", () => {
    ...
    expect(result.issues[0].path?.[0]?.key).toEqual("logLevel");
  });
});
