/**
 * Arguments passed to a field's `_parseInternal` method.
 */
export interface ParsableFieldParseArgs {
  value: unknown;
  data: unknown;
  invoker: Record<string, unknown> | null;
  pathArray: string[];
}
...
export interface ParsableField {
  _parseInternal(args: ParsableFieldParseArgs): LocalInputParseResult;
}
...
export interface LocalInputParser {
 ...
 */
export function buildLocalInputParser(fields: Record<string, ParsableField>): LocalInputParser {
  return {
    parse(args: LocalInputParseArgs): LocalInputParseResult {
