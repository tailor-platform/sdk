...
export const DBFieldMetadataSchema = z
  .object({
    ...
    allowedValues: z
      .array(AllowedValueSchema)
      .optional()
      .describe("Allowed values for enum fields"),
...
export const RawRelationConfigSchema = z
  .object({
 ...
 */
export const TailorDBTypeSettingsSchema = z
  .object({
...
const RecordPermissionOperandSchema = z.union([
  ...
  z.object({ record: z.string() }).strict(),
  z.object({ oldRecord: z.string() }).strict(),
  z.object({ newRecord: z.string() }).strict(),
]);
...
const GqlPermissionPolicySchema = z
  .object({
...
const TailorDBMigrationConfigSchema = z
  .object({
 ...
 * Normalizes gqlOperations from alias ("query") to object format.
 */
export const TailorDBServiceConfigSchema = z
  .object({
    files: z.array(z.string()).describe("Glob patterns for TailorDB type definition files"),
    ignores: z
      .array(z.string())
      .optional()
      .describe("Glob patterns to exclude from type discovery"),
