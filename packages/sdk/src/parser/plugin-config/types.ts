 ...
 * Plugins can optionally specify a `kind` identifier for type retrieval via getGeneratedType().
 */
export interface PluginGeneratedType extends TailorDBTypeForPlugin {
  /**
   * Kind identifier for getGeneratedType() API.
   * Used to retrieve this type via getGeneratedType(sourceType, kind).
   * Only applicable for type-attached plugins; standalone plugins use type name directly.
   * @example "request", "step", "approval", "rework"
   */
  kind?: string;
}
