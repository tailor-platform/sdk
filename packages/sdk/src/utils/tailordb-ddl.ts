...
function columnDefinition(tableName: string, fieldName: string, field: DDLFieldConfig): string {
  ...
  if (field.serial) {
    if (field.type === "integer") {
      const range = [`START WITH ${field.serial.start}`];
      if (field.serial.maxValue !== undefined) range.push(`MAXVALUE ${field.serial.maxValue}`);
      parts.push(`GENERATED BY DEFAULT AS IDENTITY (${range.join(" ")})`);
      return parts.join(" ");
    }
...
export function generateTableDDL(table: DDLTableConfig): string[] {
  ...
  for (const [fieldName, field] of Object.entries(table.fields)) {
    ...
    if (field.serial && field.type !== "integer") {
      ...
      if (field.serial.maxValue !== undefined) range.push(`MAXVALUE ${field.serial.maxValue}`);
      sequences.push(
        `CREATE SEQUENCE IF NOT EXISTS ${identifier(sequenceName(table.name, fieldName))} ${range.join(" ")}`,
      );
    }
