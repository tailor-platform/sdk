...
A workspace is deleted only when its whole name matches a `--name` regular expression and it was created at least `--older-than` ago, optionally scoped with `--folder-id` (repeatable) and/or `--personal` (workspaces in no organization and no folder; combines with `--folder-id` as OR) and kept with `--exclude`. Delete-protected workspaces are always kept, the command aborts without deleting anything when more workspaces match than `--limit` allows or when `--folder-id` resolves to an empty value, and `--dry-run` lists the candidates first.
