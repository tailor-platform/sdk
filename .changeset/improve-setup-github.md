...
- In CI, `tailor-sdk apply` no longer auto-generates a missing app `id` in `tailor.config.ts` — it fails with instructions instead, because an id minted per CI run would make every deploy look like a brand-new app. Run `tailor-sdk setup github` (or `apply` locally) once and commit the injected `id`.
