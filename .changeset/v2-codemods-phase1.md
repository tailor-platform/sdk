...
- `v2/cli-rename` applies the v2 CLI naming conventions: `tailor-sdk crash-report` becomes `tailor-sdk crashreport`, and the camelCase long options `--executionId`, `--executorName`, `--jobId` become `--execution-id`, `--executor-name`, `--job-id` across `package.json` scripts, shell scripts, CI YAML, and Markdown.
