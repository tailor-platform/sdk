...
Because the expiry is recorded on the workspace rather than derived from its name, anything able to write the workspace's metadata can bring its deletion forward — and on the Platform, writing a workspace's metadata is a lesser permission than deleting it. `--expired` therefore requires `--organization-id` or `--folder-id`; `--name-prefix` and `--name-regex` still apply on top and are worth keeping in a shared organization.
...
tailor workspace prune --expired --organization-id <id> --dry-run
tailor workspace prune --expired --organization-id <id> --yes
```
