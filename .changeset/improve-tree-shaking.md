...
- Add a selective `"sideEffects"` allow-list in `package.json`, marking most of `dist/` as side-effect-free while preserving specific files that must still be treated as having side effects.
...
No public API changes; this primarily improves bundling and tree-shaking behavior.
